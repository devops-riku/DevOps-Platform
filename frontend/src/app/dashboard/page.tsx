...
export default function DashboardPage() {
    ...
    useEffect(() => {
        ...
        if (hasDeploying && selectedWorkspace) {
            interval = setInterval(() => {
                fetchProjects(selectedWorkspace.id, true)
            }, 5000)
        }
    ...
    useEffect(() => {
        if (selectedProject) {
            ...
            if (updated && (
                ...
                updated.container_id !== selectedProject.container_id ||
                updated.container_port !== selectedProject.container_port
            )) {
    ...
    const fetchProjects = async (workspaceId: string, silent = false) => {
        if (!silent) setIsRefreshingProjects(true)
        try {
            const token = getAuthToken()
            if (!token) return
            const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api/v1'
            ...
            if (silent) {
                const prevStatusMap = new Map(projects.map(p => [p.id, p.status]))
                const anyChangedToActive = response.data.some((p: any) => 
                    p.status === 'active' && prevStatusMap.get(p.id) === 'deploying'
                )
                if (anyChangedToActive) {
                    toast.success("Deployment Active: Infrastructure Cluster Synchronized")
                    fetchWorkspaces() // Refresh sidebar metrics
                }
            }
            ...
            if (!silent) toast.error("Cloud Substrate Sync Failed")
        } finally {
            if (!silent) setIsRefreshingProjects(false)
        }
    }
    ...
    const handleSaveAndRedeploy = async () => {
        ...
        try {
            ...
            await axios.patch(`${apiUrl}/projects/${selectedProject.id}`, editedSpecs, {
                headers: { Authorization: `Bearer ${token}` }
            })
            
            // 2. Trigger redeploy
            await axios.post(`${apiUrl}/projects/${selectedProject.id}/redeploy`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            })
            
            toast.success("Orchestration sequence updated and initiated")
            ...
            setSelectedProject(prev => prev ? { ...prev, ...editedSpecs, status: 'deploying' } : null)
                    ...
                    {view === 'overview' ? (
                                        ...
                                        <div key={dep.id} className="px-8 py-6 border-b border-white/5 hover:bg-white/[0.02] flex items-center justify-between group">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[10px] text-slate-500 font-bold tracking-tight">Manual Redeploy · {new Date(dep.updated_at || dep.created_at || 0).toLocaleString()}</p>
                                                </div>
                                            </div>
                        ...
                        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500 pb-20">
                            ...
                            <div className="flex flex-wrap gap-4 px-2">
                                ...
                                <button 
                                    onClick={() => selectedWorkspace && fetchProjects(selectedWorkspace.id)}
                                    disabled={isRefreshingProjects}
                                    className="flex items-center gap-2 bg-white/5 hover:bg-white/10 px-4 py-2.5 rounded-lg border border-white/5 text-[11px] font-black uppercase tracking-widest text-[#06B6D4] transition-all active:scale-95"
                                >
                                    <RefreshCw className={`w-3.5 h-3.5 ${isRefreshingProjects ? 'animate-spin' : ''}`} />
                                    Sync Cluster
                                </button>
                            </div>
                                ...
                                >
                                    ...
                                    {!isMinimized && (
                                        <>
                                            <div 
                                                ref={logScrollRef}
                                                className={`p-7 font-mono text-[10px] space-y-2 bg-black/40 select-scrollbar overflow-y-auto ${isFullscreen ? 'h-[calc(100vh-160px)]' : 'min-h-[220px] max-h-[300px]'}`}
                                            >
                                    ...
                                    {!isEditingSpecs ? (
                                        <button 
                                            onClick={() => {
                                                ...
                                                setEditedSpecs({
                                                    ...
                                                    container_port: selectedProject.container_port || '80'
                                                })
                                            }}
                                            className="px-5 py-2.5 bg-white/5 hover:bg-white/10 text-white font-black text-[10px] uppercase tracking-widest rounded-lg border border-white/5 transition-all"
                                        >
                                        ...
                                        <div className="flex gap-2">
                                            <button 
                                                ...
                                                className="px-5 py-2.5 bg-white/5 hover:bg-white/10 text-slate-400 font-black text-[10px] uppercase tracking-widest rounded-lg border border-white/5 transition-all"
                                            >
                                                Discard
                                            </button>
                                            <button 
                                                ...
                                                disabled={isSaving}
                                                className="px-5 py-2.5 bg-emerald-600 text-white font-black text-[10px] uppercase tracking-widest rounded-lg shadow-[0_0_20px_rgba(16,185,129,0.2)] hover:bg-emerald-500 transition-all flex items-center gap-2"
                                            >
                                                {isSaving ? <RefreshCw className="w-3 h-3 animate-spin" /> : <Zap className="w-3 h-3" />}
                                                {isSaving ? 'Syncing...' : 'Save & Reorchestrate'}
                                            </button>
                                        </div>
                                    )}
                                    <button
                                ...
                                <section className="glass-card rounded-lg border border-white/5 bg-[#0c0c0c] overflow-hidden">
                                    ...
                                    <div className="p-8 space-y-8">
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                            ...
                                            <div>
                                                <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest block mb-1.5">Runtime Environment</label>
                                                {!isEditingSpecs ? (
                                                    <div className="text-sm font-medium text-white uppercase tracking-tighter italic">{selectedProject.language || 'standard'} provisioning v1</div>
                                                ) : (
                                                    <select 
                                                        className="w-full bg-white/5 border border-white/10 rounded-lg p-2.5 text-sm text-white focus:border-primary/50 outline-none transition-all appearance-none cursor-pointer"
                                                        value={editedSpecs.language}
                                                        onChange={(e) => setEditedSpecs({...editedSpecs, language: e.target.value})}
                                                    >
                                                        <option value="" className="bg-[#0c0c0c]">Standard Runtime</option>
                                                        {stacks.map(stack => (
                                                            <option key={stack.id} value={stack.name} className="bg-[#0c0c0c]">{stack.name}</option>
                                                        ))}
                                                    </select>
                                                )}
                                            </div>
                                            <div>
                                                <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest block mb-1.5">Internal Cluster Port</label>
                                                {!isEditingSpecs ? (
                                                    <div className="text-sm font-mono font-bold text-cyan-500 bg-cyan-500/5 px-2 py-1 rounded inline-block">
                                                        {selectedProject.container_port || '80'}
                                                    </div>
                                                ) : (
                                                    <input 
                                                        type="text"
                                                        className="w-full bg-white/5 border border-white/10 rounded-lg p-2.5 text-sm font-mono text-white focus:border-primary/50 outline-none transition-all placeholder:text-slate-700 font-bold"
                                                        value={editedSpecs.container_port}
                                                        onChange={(e) => setEditedSpecs({...editedSpecs, container_port: e.target.value})}
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                                ...
                                                <div>
                                                    ...
                                                    {!isEditingSpecs ? (
                                                        ...
                                                        <input 
                                                            type="text"
                                                            className="w-full bg-white/5 border border-white/10 rounded-lg p-3.5 text-sm text-white focus:border-primary/50 outline-none transition-all placeholder:text-slate-700 font-bold"
                                                            value={editedSpecs.branch}
                                                            onChange={(e) => setEditedSpecs({...editedSpecs, branch: e.target.value})}
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                                <div>
                                                    ...
                                                    {!isEditingSpecs ? (
                                                        ...
                                                        <input 
                                                            type="text"
                                                            className="w-full bg-black/40 border border-white/10 rounded-lg p-3.5 text-[10px] font-mono text-cyan-500 focus:border-cyan-500/50 outline-none transition-all"
                                                            value={editedSpecs.build_command}
                                                            onChange={(e) => setEditedSpecs({...editedSpecs, build_command: e.target.value})}
                                                ...
                                                <div>
                                                    ...
                                                    {!isEditingSpecs ? (
                                                        ...
                                                        <input 
                                                            type="text"
                                                            className="w-full bg-black/40 border border-white/10 rounded-lg p-3.5 text-[10px] font-mono text-slate-400 focus:border-primary/50 outline-none transition-all"
                                                            value={editedSpecs.start_command}
                                                            onChange={(e) => setEditedSpecs({...editedSpecs, start_command: e.target.value})}
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                                <div>
                                                    ...
                                                    {!isEditingSpecs ? (
                                                        ...
                                                        <input 
                                                            type="text"
                                                            className="w-full bg-black/40 border border-white/10 rounded-lg p-3.5 text-[10px] font-mono text-slate-500 focus:border-primary/50 outline-none transition-all"
                                                            value={editedSpecs.root_dir}
                                                            onChange={(e) => setEditedSpecs({...editedSpecs, root_dir: e.target.value})}
                                                            placeholder="./"
                                                        />
                                                    )}
                                                </div>
